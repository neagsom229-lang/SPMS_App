...
app.post(
  ...
  async (req, res) => {
    const { username, password, fullname, role_id, email, phone, status } =
      req.body;
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logUserActivity(req.body.user_id || 1, "Updated user", "tbl_users", id);
...
app.delete(
  ...
  async (req, res) => {
    ...
    if (id == 1) {
      return res
        .status(400)
        .json({ error: "Cannot delete the main admin user" });
    }

    try {
      const result = await db.query("DELETE FROM tbl_users WHERE userid = $1", [
        id,
      ]);
      if (result.rowCount === 0) {
        return res.status(404).json({ error: "User not found" });
      }
      logUserActivity(req.body.user_id || 1, "Deleted user", "tbl_users", id);
